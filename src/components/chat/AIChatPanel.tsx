...
const AIChatPanel = ({ tasks }: AIChatPanelProps) => {
  ...
  const buildContext = () => {
    ...
    return `Current board state: ${tasks.length} total tasks. To-Do: ${todo}, In Progress: ${inProgress}, Completed: ${completed}. Overdue: ${overdue}. High priority incomplete: ${high}. Tasks: ${JSON.stringify(tasks.map((t) => ({ title: t.title, status: t.status, priority: t.priority, category: t.category, due_date: t.due_date })))}`;
  };
